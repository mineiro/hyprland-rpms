...
install -Dpm0644 target/completions/_%{name} %{buildroot}%{zsh_completions_dir}/_%{name}
install -Dpm0644 target/completions/%{name}.bash %{buildroot}%{bash_completions_dir}/%{name}
install -Dpm0644 target/completions/%{name}.fish %{buildroot}%{fish_completions_dir}/%{name}.fish
install -Dpm0644 target/man/%{name}.1 %{buildroot}%{_mandir}/man1/%{name}.1
