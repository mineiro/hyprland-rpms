...
- Earlier maintenance handoff (2026-07-24, upstream drift):
  ...
  - Snapshot/`manual`-tracked packages were left untouched in this first pass
    (the Astal stack at `snapshot_date 20260519` and `material-symbols-fonts`
    at `0^git20260515`); they were refreshed immediately afterwards in the
    follow-up snapshot batch recorded above.
...
- Latest maintenance handoff (2026-06-30):
  - Commit `27db241` (`Update Hyprland maintenance package set`) is pushed to
    `origin/main`.
  - Routine upstream maintenance updated and published:
    - `app2unit` `1.4.2` (COPR build `10662392`)
    - `swayosd` `0.3.2` (COPR build `10662393`)
    - `caelestia-cli` `1.1.0` (COPR build `10662394`)
    - `dart-sass` `1.101.0` (COPR build `10662395`)
    - `uwsm` `0.26.6` (COPR build `10662396`)
    - `aquamarine` `0.12.1` (COPR build `10662397`)
    - `glaze` `7.8.3` (COPR build `10662398`)
    - `hyprtoolkit` `0.5.4-4` same-version rebuild for `aquamarine 0.12.1`
      (COPR build `10662399`)
    - `hyprland` `0.55.4` (COPR build `10662400`)
    - `hyprland-plugins` `0.55.0-3` same-source rebuild targeting
      `hyprland 0.55.4` (COPR build `10662401`)
  - All listed COPR builds succeeded in `mineiro/hyprland`.
  - Local validation before commit:
    - `./scripts/check-upstream-versions.sh --changed-only` clean after edits
    - SRPM generation passed for all changed/rebuilt packages
    - `make check-upgrade UPGRADE_BASE_REF=origin/main` passed
    - Fedora 43/44/rawhide x86_64 focused `mock --chain` passed against the
      `mineiro/hyprland` COPR dependency repo
  - COPR publish order used:
    - leaf/support batch: `app2unit`, `swayosd`, `caelestia-cli`,
      `dart-sass`, `uwsm`
  ...
  - Maintenance playbook has been documented in `docs/release-process.md` and
    COPR batch-order examples in `docs/copr-setup.md`.
...
- `packages/hyprlock/` has been updated to the latest upstream release (`0.9.5`), adapted from the `solopasha/hyprlandRPM` baseline; local SRPM and clean Fedora 43/44/rawhide x86_64 `mock --rebuild` pass using the `mineiro/hyprland` COPR repo for dependencies, and COPR build `10346411` passed in `mineiro/hyprland`.
...
- `packages/hyprland-plugins/` is a split-plugin bundle package. It currently
  uses upstream tag `v0.55.0`, version-locks plugin subpackages to the active
  packaged Hyprland patch release, and has been rebuilt successfully for
  `hyprland 0.55.4` as COPR build `10662401`. Older `0.53.x` plugin RPMs are
  treated as legacy upgrade-path artifacts.
...
- `packages/awww/` has been added as a wallpaper daemon package for Wayland (Codeberg successor/rename of `swww`), locally validated via SRPM + clean `mock --rebuild` on Fedora 43/44/rawhide and onboarded to COPR (`mineiro/hyprland`) with successful Fedora 43/44/rawhide builds. It was pinned to the upstream `main` snapshot (`2c86d41d`, `Version: 0.11.2^git20260212`) while the latest tag still built the legacy `swww`/`swww-daemon` binaries; upstream has since tagged releases that ship the renamed binaries, so the package now tracks stable tags (currently `0.12.1`) and is no longer snapshot-pinned.
- `packages/swayosd/` has been updated to upstream release `0.3.2`
  (`ErikReider/SwayOSD`), locally validated via SRPM + Fedora 43/44/rawhide
  x86_64 `mock --chain`, and published in COPR as build `10662393`. It uses
  vendored Rust crates (`Source1`) generated during SRPM creation so builds
  work offline in mock/COPR.
...
- `packages/app2unit/` has been updated to upstream release `1.4.2`
  (`Vladimir-csp/app2unit`), locally validated via SRPM + Fedora 43/44/rawhide
  x86_64 `mock --chain`, and published in COPR as build `10662392`. It is a
  noarch POSIX shell launcher package with a generated `scdoc` man page,
  runtime `Requires: systemd` / `xdg-utils`, and `Recommends:
  xdg-terminal-exec >= 0.13.0`.
...
- The complete Maple Mono set has been added at upstream release `v7.9` (`subframe7536/maple-font`) as four source packages producing sixteen `noarch` font RPMs — full coverage of upstream's family matrix, matching ArchLinuxCN and a superset of Homebrew (8 casks), Scoop (5) and the AUR (3). Upstream has two independent axes. The **feature cut** axis (`base`, `NL` no-ligature, `Normal` JetBrains-Mono-like preset freezing `cv01`/`cv02`/`cv33`-`cv36`/`cv61`/`cv62`/`ss05`-`ss08` on, and `Normal NL`) produces genuinely distinct font families, so all four become subpackages of one spec via `%fontpkg -a`. The **glyph set** axis is split across source packages instead, because it drives download size:
  - `packages/maple-mono-fonts/` — base glyph set, ~7 MB sources, 16 MB installed
  - `packages/maple-mono-nf-fonts/` — Nerd Fonts `3.4.0` complete icon set, ~81 MB sources, 145 MB installed
  - `packages/maple-mono-cn-fonts/` — Chinese coverage at twice the Latin advance width, ~537 MB sources, 1.13 GB installed
  - `packages/maple-mono-nf-cn-fonts/` — both, ~608 MB sources, 1.25 GB installed

  Validation: `make check-specs` and `make check-upgrade` pass; all four SRPMs generate locally; `mock --rebuild` passes for all four on Fedora 43, 44 and rawhide x86_64, producing all sixteen binary RPMs on each. The aarch64 leg was run on Fedora 44 for `maple-mono-fonts` and `maple-mono-nf-fonts` only. Per an explicit decision on 2026-07-25, **local aarch64 mock validation is skipped in this repo**: qemu emulation is disproportionately slow (a ~1.25 GB CJK payload runs for hours to produce byte-identical `noarch` output), so x86_64 is the local gate and aarch64 is left to COPR's native builders. This does not change the COPR rollout, which still targets all six chroots.

  COPR rollout completed on 2026-07-25 in `mineiro/hyprland`, all four packages green on all six chroots (Fedora 43/44/rawhide x x86_64/aarch64): `maple-mono-fonts` `10773295`, `maple-mono-nf-fonts` `10773296`, `maple-mono-cn-fonts` `10773370`, `maple-mono-nf-cn-fonts` `10773371`. All sixteen RPMs verified present in the published repodata. The two light packages were built and confirmed first, so the `make_srpm` path (SCM entries, `#/` source fragments, the `%fontpkgname` override) was proven on COPR's builders before the ~570 MB CJK source RPMs were attempted.

  Storage footprint, since COPR publishes no per-project quota: the whole set is 865 MB of compressed RPMs per chroot, ~5.1 GB across all six. Of that, the two non-CJK packages are only ~13 MB per chroot; the CJK pair is ~98% of the weight. For reference, `mineiro/hyprland` already held 1127 packages / 3.6 GB in `fedora-44-x86_64` before this addition. Copr's documented retention is one build per RPM `Name:` kept indefinitely, with older and failed builds pruned after 14 days — so this is steady-state cost, not growth.

  Packaging decisions worth keeping: upstream publishes 44 zip assets per release (4 cuts x 11 glyph-set/format combinations) instead of a source tarball, so each spec points `Source0`-`Source3` at release assets and `%prep` uses `%setup -c -T` plus four `unzip -d` calls, because every asset is a flat archive with a colliding top-level `LICENSE.txt` / `config.json`; the asset filenames carry no version, so every source uses a `#/` fragment to store a versioned local copy and keep a stale download from surviving a version bump; the hinted static TTF assets are packaged rather than the variable, OTF or woff2 ones, since those are alternate encodings of the *same* families and would put duplicate families in fontconfig (and the Nerd Fonts cuts are static-only upstream anyway). Splitting the glyph-set axis keeps a 4 MB rebuild from dragging ~1.3 GB of sources along, and lets the CJK half be skipped in COPR independently.
- Do not describe the Maple Mono `CN` cuts as "CJK" or "Chinese/Japanese/Korean" fonts. Measured coverage of `MapleMono-CN-Regular.ttf` at `v7.9`: 22,731 mapped codepoints, of which 20,976 are CJK unified ideographs (U+4E00-U+9FFF), 93 hiragana, 96 katakana, 43 bopomofo, 46 CJK punctuation and 18 fullwidth forms — and zero hangul, neither syllables (U+AC00-U+D7AF) nor jamo (U+1100-U+11FF), plus no CJK extension A. The 2:1 width claim does hold: Latin advance is 600 against 1200 for ideographs. The specs and READMEs were corrected to say "Chinese glyph coverage" after an initial draft wrongly claimed Korean support.
- Font-macro name collision learned while packaging Maple Mono: `%fontpkgname<N>` is derived from the family name through the "WPF font selection model" simplifications in `fedora.srpm.fonts`, which strip trailing weight, width, slant *and* `normal` tokens. `Maple Mono Normal` therefore collapses to `maple-mono-fonts` and silently collides with the base family, so `packages/maple-mono-fonts/maple-mono-fonts.spec` sets `%global fontpkgname2 maple-mono-normal-fonts` explicitly. The other `Normal` cuts (`Maple Mono Normal NL`, `Maple Mono Normal NF`, ...) are safe because `normal` is not the trailing token there. Always check generated names before trusting them.
- Adding a second font package exposed a latent macro-leak bug in `packages/material-symbols-fonts/material-symbols-fonts.spec`. Font specs let `%fontpkg` generate `Name:`, which happens *after* the `Source0` line, so `%{name}` is still unset where `Source0` is declared. That is harmless for a standalone `rpmspec -P` (rpm expands source lines late), but `scripts/check-specs.sh` passes every spec to a single `rpmlint` invocation that shares one rpm macro context, so `material-symbols-fonts.spec` inherited the `Name:` of whichever font spec was parsed before it. Observed as `invalid-url Source0: maple-mono-fonts-20260724.tar.xz` when the first Maple Mono package landed, which also made the matching `.rpmlintrc` filter unused and failed `make check-specs` outright. Fixed by spelling the name out literally in that `Source0`. Rule for future font packages: do not reference `%{name}` above `%fontpkg`. This rule and the `%fontpkgname` one above are now recorded in `docs/packaging-policy.md`.
...
| `hyprlock`                    | ecosystem app                      |       13 | `COPR`      | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | latest upstream `0.9.5`, includes documented temporary bundled `sdbus-cpp`; local SRPM + clean Fedora 43/44/rawhide x86_64 `mock --rebuild` pass via `mineiro/hyprland` COPR repo deps; COPR build `10346411` passed in `mineiro/hyprland`                                                                                                                                                              |
...
| `hyprland-plugins`            | ecosystem plugins                  |       29 | `COPR`      | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | official Hyprland plugins bundle; active compatible upstream tag for the `0.55.x` stack is `v0.55.0`; packaged as split plugin subpackages with strict runtime `Requires: hyprland = 0.55.4`; same-source rebuild published as COPR build `10662401` after `hyprland` build `10662400`; older `0.53.x` plugin RPMs are legacy upgrade-path artifacts                                                                     |
...
| `awww`                        | wallpaper daemon (Wayland)         |       31 | `COPR`      | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | Codeberg successor/rename of `swww`; was snapshot-pinned while the latest tag still built legacy `swww` binaries, but upstream tags now ship the renamed `awww`/`awww-daemon` binaries, so it tracks stable tags again (currently `0.12.1`); local SRPM + clean `mock --rebuild` pass on Fedora 43/44/rawhide with vendored Rust `Source1` tarball; COPR builds passing |
| `swayosd`                     | OSD service (Wayland)              |       32 | `COPR`      | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | latest upstream `0.3.2` from `ErikReider/SwayOSD`; local SRPM + Fedora 43/44/rawhide x86_64 `mock --chain` pass; uses vendored Rust `Source1` tarball generated during SRPM creation for offline mock/COPR builds; COPR build `10662393` succeeded                                                                                                                                                       |
...
| `app2unit`                    | desktop-entry launcher helper      |       35 | `COPR`      | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | latest upstream `1.4.2` from `Vladimir-csp/app2unit`; local SRPM + Fedora 43/44/rawhide x86_64 `mock --chain` pass; COPR build `10662392` succeeded; noarch POSIX shell package with generated `scdoc` man page and helper `app2unit-open*` / `app2unit-term*` links                                  |
...
| `maple-mono-fonts`            | monospace coding font (base cuts)  |       64 | `MRH`       | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | upstream release `v7.9` from `subframe7536/maple-font`; base glyph set, 4 subpackages (`Maple Mono`, `Maple Mono NL`, `Maple Mono Normal`, `Maple Mono Normal NL`) from the hinted static TTF assets; ~7 MB sources, 16 MB installed; `%fontpkgname2` is overridden because the fonts macros strip a trailing `normal` and would collide `Maple Mono Normal` onto `maple-mono-fonts` |
| `maple-mono-nf-fonts`         | Maple Mono + Nerd Fonts icons      |       65 | `MRH`       | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | same upstream release; Nerd Fonts `3.4.0` complete icon set, 4 subpackages (`Maple Mono NF`, `Maple Mono NL NF`, `Maple Mono Normal NF`, `Maple Mono Normal NL NF`); ~81 MB sources, 145 MB installed |
