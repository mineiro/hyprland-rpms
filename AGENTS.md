...
| `glaze`                       | compatibility dependency           |        9 | `COPR`      | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | local SRPM builds (`7.2.1`); pinned to `7.x` for Hyprland `0.54.x` compatibility; COPR build `10230975` succeeded across Fedora 43/44/rawhide on both x86_64 and aarch64                                                                                                                                                                                                                                  |
| `hyprland`                    | compositor                         |       10 | `COPR`      | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | local SRPM builds (`0.54.2`) and clean standalone `mock --rebuild` pass on Fedora 43/44/rawhide x86_64 via `mineiro/hyprland`; COPR build `10188633` remains the latest published build for `0.54.1` while `0.54.2` local validation is complete and ready for COPR submission; package output includes `hyprpm`/`start-hyprland`/`hyprland-uwsm`                                                                                                         |
...
| `wl-vapi-gen`                 | Wayland VAPI generator             |       56 | `MRH`       | `ok`     | `ok`     | `ok`         | `no`           | `-`         | upstream tag `1.0.0`; noarch helper that generates Vala VAPI bindings from Wayland protocol XML and is used by `astal-wl` / `astal-river`; local SRPM + Fedora 43/44/rawhide x86_64 `mock --chain` pass                                                                           |
