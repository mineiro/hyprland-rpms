...
- `packages/swappy/` has been added at the latest upstream release (`1.8.0`, `jtheoof/swappy`), based on Fedora's existing `swappy` packaging and locally validated via SRPM + clean `mock --rebuild` on Fedora 43/44/rawhide x86_64. It updates Fedora's stale `1.5.1` baseline while keeping the spec close enough for a future Fedora dist-git bump; COPR onboarding is intentionally pending until we decide whether to carry it in `mineiro/hyprland` or upstream the update first.
...
| `glaze`                       | compatibility dependency           |        9 | `COPR`      | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | local SRPM builds (`7.1.1`); pinned to `7.x` for Hyprland `0.54.x` compatibility; COPR builds passing                                                                                                                                                                                                                                                                                                      |
| `hyprland`                    | compositor                         |       10 | `COPR`      | `ok`     | `ok`     | `ok`         | `yes`          | `ok`        | local SRPM builds (`0.54.1`) and clean standalone `mock --rebuild` pass on Fedora 43/44/rawhide x86_64 via `mineiro/hyprland`; COPR build `10188633` published successfully for `0.54.1`; package output includes `hyprpm`/`start-hyprland`/`hyprland-uwsm`                                                                                                                                                |
...
| `swappy`                      | snapshot editor (Wayland)          |       36 | `MRH`       | `ok`     | `ok`     | `ok`         | `no`           | `-`         | latest upstream `1.8.0` from `jtheoof/swappy`; based on Fedora's existing `swappy` packaging and locally validated via SRPM + clean `mock --rebuild` on Fedora 43/44/rawhide x86_64; updates Fedora's official `1.5.1` baseline while keeping the spec close to a future Fedora dist-git bump; COPR onboarding is still a policy decision                                                     |
...
10. Decide whether `swappy` should be carried in `mineiro/hyprland` or proposed upstream to Fedora first; if carrying it here, onboard it to COPR and re-run `repoclosure` after publish.
