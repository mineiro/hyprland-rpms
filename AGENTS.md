...
- `packages/hyprland-guiutils/` has been added at the latest upstream release (`0.2.1`), locally validated via SRPM + clean `mock --rebuild` on Fedora 43/44/rawhide using the `mineiro/hyprland` COPR repo for dependencies. This is the upstream successor to `hyprland-qtutils`, so `hyprland-qtutils` packaging is being deferred in favor of `hyprland-guiutils`.
...
- `packages/hyprlauncher/` has been added at the latest upstream release (`0.1.5`) and locally validated via SRPM + clean `mock --rebuild` on Fedora 43/44/rawhide using the `mineiro/hyprland` COPR repo for dependencies; it carries a small portability patch adding missing POSIX includes (`NAME_MAX`, `read`) required on newer Fedora toolchains.
...
| `hyprland-guiutils` | ecosystem app (GUI utils) | 22 | `MRH` | `ok` | `ok` | `ok` | `no` | `-` | latest upstream `0.2.1`; local SRPM + clean `mock --rebuild` pass on Fedora 43/44/rawhide via `mineiro/hyprland` COPR repo deps; upstream successor to `hyprland-qtutils` |
...
| `hyprlauncher` | ecosystem app | 24 | `MRH` | `ok` | `ok` | `ok` | `no` | `-` | latest upstream `0.1.5`; local SRPM + clean `mock --rebuild` pass on Fedora 43/44/rawhide via `mineiro/hyprland` COPR repo deps; carries temporary portability patch for missing POSIX includes (`NAME_MAX`, `read`) on newer Fedora toolchains |
...
3. Finish onboarding/building `hyprland-guiutils`, then add `hyprlauncher` to the `mineiro/hyprland` COPR project (SCM package entry) and build it for Fedora 43/44/rawhide.
