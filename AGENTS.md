...
- Run the post-COPR-publish repo checks for the latest maintenance batch if
  ...
  `material-symbols-fonts`, the four `maple-mono-*-fonts` packages, and the
  optional Astal integrations still marked `COPR pkg entry: no` in the matrix).
  Onboard the Maple Mono set in size order (`maple-mono-fonts`,
  `maple-mono-nf-fonts`, then the two CJK packages) and confirm COPR storage
  headroom before enabling `maple-mono-cn-fonts` / `maple-mono-nf-cn-fonts`:
  those two alone publish ~2.6 GB of RPMs per chroot.
